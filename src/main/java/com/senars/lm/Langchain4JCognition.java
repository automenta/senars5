...
public class Langchain4JCognition implements Cognition {
    ...
    Thought findRelevantSchema(Thought focusThought) {
        ...
        if (focusThought.metadata().type() == ThoughtType.GOAL && focusThought.metadata().origin() == ThoughtOrigin.SYSTEM) {
            ...
            // This is a goal to parse text from a previous step.
            if (PARSE_TEXT_SYMBOL.equals(focusThought.content().symbolic())) {
                return memory.findSchemaBySymbolicName(PARSE_TEXT_SCHEMA_SYMBOL)
                        .orElse(null); // Fall through
            }
        }
